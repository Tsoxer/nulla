...
class GameManager:
    def __init__(
        ...
        *,
        ...
        tts_cutoff: Optional[Callable[[], None]] = None,  # NEW (optional)
        **kwargs,
    ):
        ...
        if callable(tts_cutoff):
            self.hooks["tts_cutoff"] = tts_cutoff
    ...
    def help_text(self) -> str:
        # If GUI provided a cutoff hook, use it
        try:
            if callable(self.tts_cutoff):
                self.tts_cutoff()
        except Exception:
            pass
    ...
    def try_play(self, text: str) -> Tuple[bool, str]:
        ...
        # Optional: cut off current TTS when launching a game
        try:
            if callable(self.tts_cutoff):
                self.tts_cutoff()
        except Exception:
